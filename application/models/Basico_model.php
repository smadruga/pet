...
class Basico_model extends CI_Model {
    ...
    public function select_veterinario($data = FALSE) {

        if ($data === TRUE) {
            $array = $this->db->query('SELECT idApp_Veterinario, NomeVeterinario FROM App_Veterinario WHERE idSis_Usuario = ' . $_SESSION['log']['id']);
        } else {
            $query = $this->db->query('SELECT idApp_Veterinario, NomeVeterinario FROM App_Veterinario WHERE idSis_Usuario = ' . $_SESSION['log']['id']);
            
            $array = array();
            foreach ($query->result() as $row) {
                $array[$row->idApp_Veterinario] = $row->NomeVeterinario;
            }
        }
